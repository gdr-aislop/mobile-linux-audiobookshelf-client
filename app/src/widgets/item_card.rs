...
pub fn build(size: i32, item: &Item, subtitle: &str, on_play: &Rc<dyn Fn(PlayRequest)>, wrap_title: bool) -> gtk4::Widget {
...
pub(crate) mod tests {
    ...
    pub(crate) fn run() {
        ...
        let widget = build(132, &item, "Andy Weir · 1.0h", &on_play, false);
        let button = widget.clone().downcast::<gtk4::Button>().expect("item_card::build returns a GtkButton");
        button.emit_clicked();
    ...
    pub(crate) fn run_wrap_title_shows_the_full_title_without_ellipsizing() {
        ...
        let widget = build(108, &item, "Andy Weir · 1.0h", &on_play, true);
        let title_label = title_label_of(&widget);
